...
def main():
    ...
    for seed in seed_list:
        ...
        # # 自动查找exp编号最大的exp文件夹
        # exp_folders = [d for d in os.listdir(args.weight_folder) if d.startswith('exp') and os.path.isdir(os.path.join(args.weight_folder, d))]
        # exp_numbers = [int(d[3:]) for d in exp_folders if d[3:].isdigit()]
        # if len(exp_numbers) == 0:
        #     raise ValueError(f"在{args.weight_folder}下未找到exp文件夹！")
        # max_exp = max(exp_numbers)
        weight_path = os.path.join(args.weight_folder, f"MESI", "best_model_epoch.pth")
