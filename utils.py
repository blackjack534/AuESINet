...
def graph_collate_func(x):
    ...
    padded_protein_feat_aug = []
    protein_masks_aug = []  # 用于存储每个蛋白质的mask
    ...
    for feat in p_aug:
        ...
        feat = feat.unsqueeze(0)
        padded_protein_feat_aug.append(feat)
        protein_masks_aug.append(mask)

    padding_protein_mask_aug = torch.stack(protein_masks_aug)
    ...
    for drug in d_aug:
        actual_node_feats = drug.ndata.pop('h')
        num_actual_nodes = actual_node_feats.shape[0]
        node_counts_aug.append(num_actual_nodes)  # 记录实际节点数
        num_virtual_nodes = max_drug_lenth - num_actual_nodes
        virtual_node_bit = torch.zeros([num_actual_nodes, 1])
        actual_node_feats = torch.cat((actual_node_feats, virtual_node_bit), 1)
        drug.ndata['h'] = actual_node_feats
        ...
        drug.add_nodes(num_virtual_nodes, {"h": virtual_node_feat})
        drug = drug.add_self_loop()
        drug_feat_aug.append(drug)
    drug_feat_aug = dgl.batch(drug_feat_aug)
    ...
    return drug_feat, torch.cat(padded_protein_feat, dim=0), drug_feat_aug, torch.cat(padded_protein_feat_aug, dim=0), torch.tensor(y), padding_drug_mask, padding_protein_mask
